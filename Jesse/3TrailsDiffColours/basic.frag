...
uniform int paletteN1;
uniform int paletteN2;
uniform vec2 mousePos;
uniform sampler2D tex0;
uniform sampler2D tex1;
uniform sampler2D tex2;

vec3 gradientMap(float v, sampler2D palettte){
  v = mod(v, 1.0);
  return texture2D(palette, vec2(v, 0.5)).rgb;
}

vec3 gradientMapLinearInterpolation(float v, float pN, sampler2D palette){
  ...
  vec3 cx1 = gradientMap(x1 / (pN - 1.0), palette);
  vec3 cx2 = gradientMap(x2 / (pN - 1.0), palette);
...
void main() {
  ...
  vec3 camLagged1 = texture2D(tex2, tv2).rgb;
   ...
   cam = gradientMapLinearInterpolation(dot(cam, vec3(0.333)), float(paletteN), palette);
   camLagged = gradientMapLinearInterpolation(dot(camLagged, vec3(0.333)), float(paletteN1), palette1);
   camLagged1 = gradientMapLinearInterpolation(dot(camLagged1, vec3(0.333)), float(paletteN2), palette2);
